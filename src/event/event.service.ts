...
export class EventService {
  ...
  async update(eventId: string, updateEventDto: UpdateEventDto, userId: string): Promise<Event> {
    ...
    async findAll(query: string, categoryId: string, limit = 6, page = 1): Promise<{ data: Event[], totalPages: number }> {
        ...
        const events = await this.eventModel
          .find({ ...titleCondition, ...categoryCondition })
          .sort({ createdAt: 'desc' })
          .skip(skipAmount)
          .limit(limit)
          .populate('organizer', '_id firstName lastName email')
        ...
        const eventsCount = await this.eventModel.countDocuments({ ...titleCondition, ...categoryCondition }).exec();
